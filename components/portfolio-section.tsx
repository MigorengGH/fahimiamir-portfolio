    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        {filteredProjects.map((project: any, index: number) => {
          ...
          return (
            <AnimatedReveal key={project._id || index} delay={index * 150} direction="up" className="h-full">
              <div
                className="group bg-secondary rounded-xl md:rounded-2xl border border-border overflow-hidden hover:border-accent transition-all duration-300 hover:shadow-xl hover:shadow-accent/10 flex flex-col h-full"
              >
                <div className="relative aspect-[16/9] overflow-hidden bg-background flex-shrink-0">
                  <AnimatedReveal delay={index * 150 + 200} direction="none" className="w-full h-full">
                    <img
                      src={imageSrc}
                      alt={project.title}
