...
import { Mail, Phone, MapPin, Sparkles, Download } from 'lucide-react'
...
interface ProfileSidebarProps {
  data?: typeof profileData & { resumeUrl?: string, avatar?: any }
}
...
export function ProfileSidebar({ data = profileData }: ProfileSidebarProps) {
  return (
    <aside className="sidebar-glass w-full lg:w-[340px] xl:w-[380px] rounded-2xl p-6 lg:sticky lg:top-6 h-fit flex-shrink-0">
      {/* Avatar */}
      <div className="flex flex-col items-center text-center mb-6">
        <div className="avatar-ring mb-5">
          <div className="avatar-inner relative w-full h-full rounded-[26px] overflow-hidden">
            {data.avatar ? (
              <Image
                src={urlFor(data.avatar).width(600).height(600).url()}
                alt={data.name}
                fill
                className="object-cover"
              />
            ) : (
              <div className="w-full h-full bg-gradient-to-br from-sky-900/60 to-blue-900/60 flex items-center justify-center text-4xl font-bold text-sky-400" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                FA
              </div>
            )}
          </div>
      ...
      <div className="space-y-1 my-5">
        <a href={`mailto:${data.email}`} className="contact-row group">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-white/70 group-hover:text-sky-400 transition-colors truncate">{data.email}</p>
          </div>
        </a>
        ...
        <a href={`tel:${data.phone.replace(/\s/g, '')}`} className="contact-row group">
          ...
          <div>
            ...
            <p className="text-sm text-white/70 group-hover:text-sky-400 transition-colors">{data.phone}</p>
          </div>
        </a>
      ...
      <div className="flex items-center justify-center gap-3 mt-5 mb-6">
        ...
        <a href={`mailto:${data.email}`} className="social-btn" aria-label="Email">
      ...
      <a
        ...
        download
        className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-medium text-sm transition-all shadow-[0_0_15px_rgba(14,165,233,0.3)] hover:shadow-[0_0_20px_rgba(14,165,233,0.5)]"
      >
